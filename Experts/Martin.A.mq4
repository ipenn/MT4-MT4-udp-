...
void OnTick()
{
   ...
   int cpt,ordersBuyStop,orderSellStop,ordersBuy,ordersSell,optype,lastBuyTime,lastSellTime;
   ...
   if(ordersBuy > 0)
   {
      sendMsg = Symbol()+",B," + ordersBuy + ","+lastBuyTime;
      SocketSend(sendMsg);
   }
   if(ordersSell > 0)
   {
      sendMsg = Symbol()+",S," + ordersSell + ","+lastSellTime;
      SocketSend(sendMsg);
   }
